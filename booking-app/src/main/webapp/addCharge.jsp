	...
	<div class="wrapper">
		<div class="inner">
			...
			<form action="${action}" method="post">
				...
				<p style="color: red" align="center" id="errmsg">${errormsg}</p>
				<input type="hidden" class="form-control" name="id" id="id"
					placeholder="id" value="${charge.id}">
						...
						<select class="form-select bg-info text-dark" id="fromLocation"
							...
							<c:forEach var="options" items="${locationList}"
								...
								<option value="${options.id}"
									${options.id == charge.fromLocation ? 'selected' : ''}>${options.location}
									[${options.id}]</option>
							</c:forEach>
						</select>
					</div>
					...
					<div class="col-sm-5">
						<select class="form-select bg-info text-dark" id="toLocation"
							...
							<c:forEach var="options" items="${locationList}"
								...
								<option value="${options.id}"
									${options.id == charge.toLocation ? 'selected' : ''}>${options.location}
									[${options.id}]</option>
							</c:forEach>
						</select>
					</div>
				...
				<div class="row element-margin">
					...
					<div class="col-sm-8">
					<input type="number" name="freight" id="freight" value="${charge.freight}" required> 
					</div>
				</div>
				...
				<div class="row element-margin">
					...
					<div class="col-sm-8">
					<input type="number" name="loading" id="loading" value="${charge.loading}" required> 
					</div>
				</div>
				...
				<div class="row element-margin">
					...
					<div class="col-sm-8">
					<input type="number" name="fuel" id="fuel" value="${charge.fuel}" required> 
					</div>
				</div>
			...
			<%-- 	<select name="chargetype" id="chargetype" class="form-control"
					...
					<option value="LOADING CHARGES"
						${charge.chargetype == 'LOADING CHARGES' ? 'selected' : ''}>Loading
						Charges</option>
					<option value="FUEL CHARGES"
						${charge.chargetype == 'FUEL CHARGES' ? 'selected' : ''}>Fuel
						Charges</option>
				</select> <br> --%>
				<%-- <div class="form-row">
					<input type="number" class="form-control" name="value" id="value"
						placeholder="Value" step="0.001" pattern="^\d+(?:\.\d{1,3})?$"
